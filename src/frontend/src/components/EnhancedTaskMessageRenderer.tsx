...
interface EnhancedTaskMessageRendererProps {
  content: string;
}
...
export default function EnhancedTaskMessageRenderer({ content }: EnhancedTaskMessageRendererProps) {
  const tasks = parseAdvancedTaskList(content);

  // If not an advanced task list, fall back to the original TaskMessageRenderer
  if (!tasks) {
